...
import { parseConfigRestaurante } from '@/lib/restaurante/config';
import {
  inicioReserva,
  turnoTieneHueco,
  validarComensales,
...
/** Convierte "yyyy-mm-dd" en una fecha local a medianoche (o null si inválida). */
function fechaLocalDesdeISO(fechaISO: string): Date | null {
...
export interface DisponibilidadTurnoDTO {
  turnoId: string;
  nombre: string;
  inicio: string;
  fin: string;
...
/**
 * Disponibilidad de turnos de un restaurante en una fecha (para el flujo de
 * reserva pública). Devuelve las plazas libres de cada turno ese día.
 */
export async function disponibilidadRestaurante(
  negocioSlug: string,
  fechaISO: string,
): Promise<DisponibilidadTurnoDTO[]> {
  const negocio = await getNegocioPorSlug(negocioSlug);
  if (!negocio) return [];
  const fecha = fechaLocalDesdeISO(fechaISO);
  if (!fecha) return [];

  const config = await getConfigRestaurantePublica(negocio.id);
  const plazas = await getDisponibilidadRestaurante(negocio.id, config, fecha);

  return plazas.map((p: PlazasTurno) => ({
    turnoId: p.turno.id,
    nombre: p.turno.nombre,
    inicio: p.turno.inicio,
    fin: p.turno.fin,
    disponibles: p.disponibles,
  }));
}

export interface DatosReservaRestaurante {
  negocioSlug: string;
...
/**
 * Crea una reserva de restaurante: valida aforo del turno, número de comensales
 * y consentimiento, registra (o reutiliza) el cliente y confirma la mesa.
 * Las reservas de restauración no requieren depósito por defecto.
 */
export async function crearReservaRestaurante(
  datos: DatosReservaRestaurante,
): Promise<ResultadoReserva> {
  if (!datos.consentimientoRgpd) {
    return { ok: false, error: 'Debes aceptar la política de privacidad.' };
  }
  ...
  // 1. Negocio + configuración de restauración.
  const { data: negocio } = await supabase
    .from('negocios')
    .select('id, nombre, config_restaurante_json')
    .eq('slug', datos.negocioSlug)
    .maybeSingle<{
      id: string;
      nombre: string;
  ...
  const config = parseConfigRestaurante(negocio.config_restaurante_json);

  // 2. Valida comensales y turno.
  const chequeoComensales = validarComensales(
    datos.comensales,
    config.tamanoMaxGrupo,
  );
  if (!chequeoComensales.ok) {
    return { ok: false, error: chequeoComensales.error };
  }

  const turno = config.turnos.find((t) => t.id === datos.turnoId);
  if (!turno) return { ok: false, error: 'Turno no disponible.' };

  const fecha = fechaLocalDesdeISO(datos.fechaISO);
  if (!fecha) return { ok: false, error: 'Fecha no válida.' };

  const inicio = inicioReserva(fecha, turno);
  if (inicio.getTime() < Date.now()) {
    return { ok: false, error: 'Esa fecha y turno ya han pasado.' };
  }
  const fin = new Date(inicio.getTime() + config.duracionMesaMin * 60_000);

  // 3. Comprueba el aforo real del turno (evita sobreventa concurrente).
  const plazas = await getDisponibilidadRestaurante(negocio.id, config, fecha);
  const plazaTurno = plazas.find((p) => p.turno.id === turno.id);
  if (
    !plazaTurno ||
    !turnoTieneHueco(plazaTurno.aforo, plazaTurno.reservados, datos.comensales)
  ) {
    return {
      ok: false,
      error: 'Ese turno se ha completado. Prueba con otro turno o fecha.',
    };
  }

  // 4. Cliente: reutiliza por email o lo crea.
  let clienteId: string;
  const emailNorm = datos.cliente.email.trim().toLowerCase();
  const { data: existente } = await supabase
    .from('clientes')
  ...
  // 5. Crea la reserva confirmada con el número de comensales.
  const { data: cita, error: errCita } = await supabase
    .from('citas')
    .insert({
      negocio_id: negocio.id,
  ...
  await emailConfirmacionReserva({
    ...
    clienteNombre: datos.cliente.nombre.trim(),
    negocioNombre: negocio.nombre,
    servicioNombre: `${turno.nombre} · ${datos.comensales} comensales`,
    fechaTexto: inicio.toLocaleString('es-ES', {
      weekday: 'long',
      day: 'numeric',
      month: 'long',
